...
namespace ShoeStore
{
  public class Brand
  {
   ...
   public List<Store> GetStores()
   {
     ...
     SqlCommand brandQuery = new SqlCommand("SELECT stores.* FROM brands JOIN brands_stores ON (brands.id = brands_stores.brand_id) JOIN stores ON (brands_stores.store_id = stores.id) WHERE brands.id = @BrandId;", conn);
   ...
   return stores;
     ...
     public static Brand FindName(string name)
    {
      SqlConnection conn = DB.Connection();
      SqlDataReader rdr = null;
      conn.Open();

      SqlCommand cmd = new SqlCommand("SELECT * FROM brands WHERE name = @BrandName", conn);
      SqlParameter brandNameParameter = new SqlParameter();
      brandNameParameter.ParameterName = "@BrandName";
      brandNameParameter.Value = name;
